"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ContainService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const contain_entity_1 = require("../models/contain.entity");
const typeorm_2 = require("typeorm");
let ContainService = class ContainService {
    constructor(containRepository) {
        this.containRepository = containRepository;
    }
    async getAll() {
        return await this.containRepository.find();
    }
    async getByOrderId(id) {
        return await this.containRepository.findOne(id);
    }
    async add(contain) {
        await this.containRepository.insert(contain);
    }
    async addnewcontain(orderid, dishid, quan) {
        return await this.containRepository.query("INSERT INTO contain (`quantity`, `orderId`, `dishId`) VALUES ('" + quan + "','" + orderid + "','" + dishid + "') ");
    }
    async getListDishByOrderID(orderId) {
        return await this.containRepository.query("SELECT dish.name, contain.quantity, dish.price FROM contain, dish WHERE contain.dishId = dish.id AND contain.orderId = '" + orderId + "'");
    }
};
ContainService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(contain_entity_1.Contain)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], ContainService);
exports.ContainService = ContainService;
//# sourceMappingURL=contain.service.js.map